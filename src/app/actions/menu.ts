...
// ---------- Normalize a Product document to menu item shape ----------
...
// ---------- Revalidate everything that renders menus ----------
function revalidateMenuConsumers() {
  revalidatePath(MENUS_LIST_PATH);
  revalidatePath("/", "layout");
}

// ---------- Get menus by location (with resolved items) ----------
...
// Returns the raw menu (for admin edit). If a `context` is supplied,
// resolved items are attached too (for public preview surfaces).
...
// ---------- Build updates, omitting undefined ----------
...
export async function updateMenu(id: string, menuData: any) {
  try {
    ...
    Object.assign(current, buildMenuUpdates(menuData));
    await current.save();
...
export async function deleteMenu(id: string) {
  try {
    ...
    // Clean S3 before deleting the doc
